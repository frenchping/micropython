...
#define ADC_GRP_HELP    \
    "ADC_Group class is designed to scan channels in one ADC instance so that\n." \
    "users do not need to actively request the conversion. All the channels are\n." \
    "converted at background one by one and users may read the result in one shot.\n." \
    ...
    " Methods:\n" \
    "   1. ADC_Group.addch()    add channel(s) in conversion sequence.\n" \
    "   2. ADC_Group.capture()  capture the conversion results in a buffer.\n" \
    "   3. ADC_Group.read()     read the conversion results.\n" \
    "   4. ADC_Group.get()      return the captured conversion results.\n"

#define ADC_GRP_NEW_HELP_TEXT   \
    "class sensor.ADC_Group(adc_id, *, period=ADC_Group.PMODE3, average=ADC_Group.AVG16)\n" \
